...
function parseTopicWeight(weight) {
  const match = String(weight || '').match(/\d+(?:[,.]\d+)?/);
  return match ? Number(match[0].replace(',', '.')) : 0;
}

function buildNewSubjectCandidates() {
  const prioRank = { Alta: 0, Média: 1, Baixa: 2, 'Muito Baixa': 3 };

  return flattenAllSubtopics()
    .filter(({ sub }) => Number(sub.currentLevel) < 8 && !sub.nextReviewDate)
    .sort((a, b) => {
      const ap = prioRank[normalizePriority(a.sub.priority)] ?? 99;
      const bp = prioRank[normalizePriority(b.sub.priority)] ?? 99;
      if (ap !== bp) return ap - bp;

      const aw = parseTopicWeight(a.topic.enemWeight);
      const bw = parseTopicWeight(b.topic.enemWeight);
      if (aw !== bw) return bw - aw;

      return String(a.sub.name || '').localeCompare(String(b.sub.name || ''), 'pt-BR');
    });
}

function resolveNewSubjectOfDay() {
  const today = getTodayKey();
  const allItems = flattenAllSubtopics();
  const itemById = new Map(allItems.map(item => [item.sub.id, item]));
  const saved = storage.loadDailyNewSubject?.();

  if (saved && saved.date === today && saved.id && itemById.has(saved.id)) {
    const item = itemById.get(saved.id);
    if (Number(item.sub.currentLevel) < 8 && !item.sub.nextReviewDate) {
      return { date: today, item, reason: saved.reason || 'Sugestão travada para hoje' };
    }
  }

  const candidates = buildNewSubjectCandidates();
  const item = candidates[0] || null;
  const reason = item
    ? `${normalizePriority(item.sub.priority)} · incidência ${item.topic.enemWeight || '–'} · ainda não iniciado`
    : 'Nenhum assunto novo disponível no momento';

  storage.saveDailyNewSubject?.(item ? { date: today, id: item.sub.id, reason } : { date: today, id: null, reason });
  return { date: today, item, reason };
}
  ...
  card.innerHTML = `
    <div class="home-today-top">
      <div>
        ...
        <small>Plano simples para começar sem se perder no dashboard.</small>
      </div>
    ...
    <div class="home-today-grid">
      ...
      <div><span>Assunto novo</span><strong>${data.newSubject ? '1' : '0'}</strong><small>${data.newSubject ? esc(data.newSubject.sub.name) : 'nenhum pendente'}</small></div>
      <div><span>Erros</span><strong>${data.errorsToRead}</strong><small>para reler no caderno</small></div>
      <div><span>Backlog</span><strong>${data.backlog}</strong><small>fora da fila de hoje</small></div>
    </div>
    <div class="home-today-actions">
      <button class="btn btn-primary" id="homeStartNow" type="button"><i class="ph ph-play"></i> Começar agora</button>
      <button class="btn btn-soft" id="homeOpenWeeklyPlan" type="button"><i class="ph ph-calendar-plus"></i> Ver plano da semana</button>
    </div>
  `;
  return card;
}

function focusNewSubject(item) {
  if (!item) return;

  resetAllFilters();
  appState.filters.subject = item.subject.name;
  appState.filters.search = item.sub.name;
  appState.currentView = 'all';

  const subjectFilter = $('subjectFilter');
  const searchInput = $('searchInput');
  if (subjectFilter) subjectFilter.value = item.subject.name;
  if (searchInput) searchInput.value = item.sub.name;
  ...
  showToast(`Assunto novo de hoje: ${item.sub.name}.`);
}
...
function buildDailyLimitBanner(plan) {
  ...
  banner.innerHTML = `
    ...
    <div class="daily-limit-pills">
      ...
      <span class="daily-limit-pill backlog"><i class="ph ph-stack"></i> Backlog fora da fila: ${backlog}</span>
    </div>
  `;
...
function renderProgressOverview() {
  ...
  const suggestion = resolveNewSubjectOfDay();
  const newSubjectCard = document.createElement('article');
  newSubjectCard.className = 'subject-progress-card new-subject-card';

  if (suggestion.item) {
    const { subject, topic, sub } = suggestion.item;
    newSubjectCard.innerHTML = `
      <strong>Assunto novo do dia</strong>
      <div class="new-subject-title">
        <h4>${esc(topic.name)} → ${esc(sub.name)}</h4>
        <small>${esc(subject.name)} · ${esc(suggestion.reason)}</small>
      </div>
      <div class="new-subject-actions">
        <button class="btn btn-primary btn-open-new-subject" type="button" data-sub-id="${esc(sub.id)}"><i class="ph ph-book-open"></i> Ver assunto</button>
        <span class="card-tag">Aprender = avançar conteúdo novo</span>
      </div>
    `;
  } else {
    newSubjectCard.innerHTML = `
      <strong>Assunto novo do dia</strong>
      <div class="new-subject-title">
        <h4>Nenhum assunto novo pendente</h4>
        <small>Todos os itens já foram iniciados ou dominados. Foque nas revisões e no caderno de erros.</small>
      </div>
    `;
  }

  frag.appendChild(newSubjectCard);
...
function updateDashboardSubtitle(stats, totalMatches) {
  ...
  if (appState.currentView === 'studyToday') {
    ...
    if (plan.remainingItems.length > 0) {
      subtitleEl.textContent = `Mostrando ${totalMatches} de até ${plan.limit} revisão(ões) do dia. Backlog fora da fila: ${plan.backlogCount}.`;
    } else {
      subtitleEl.textContent = plan.selectedCount > 0
        ? `Você concluiu a meta diária. O backlog fica guardado para depois.`
...
function clearScheduleLocalData() {
  [
...
function buildBackupObject() {
  return {
    ...
    dailyNewSubject: storage.loadDailyNewSubject?.(),
    scheduleSettings: readApexLocalJson('apex_enem_v3_state_schedule_settings'),
    schedulePlan: readApexLocalJson('apex_enem_v3_state_schedule_week_plan'),
    scheduleHistory: readApexLocalJson('apex_enem_v3_state_schedule_history'),
    simulations: storage.loadSimulations?.() || [],
    weeklyPlan: storage.loadWeeklyPlan?.(),
  };
}
...
function buildReportText(data = buildReportData()) {
  ...
  lines.push(`- Backlog fora da fila diária: ${data.totals.backlog}`);
...
function renderReportContent(data) {
  ...
  el.innerHTML = `
    <div class="report-grid">
      ...
      <div class="report-stat"><span>Backlog</span><strong>${data.totals.backlog}</strong></div>
...
function handleJsonImport(file) {
  ...
  reader.onload = e => {
    try {
      ...
      if (parsed.weeklyPlan) storage.saveWeeklyPlan?.(parsed.weeklyPlan);
      else storage.clearWeeklyPlan?.();
      if (parsed.dailyNewSubject) storage.saveDailyNewSubject?.(parsed.dailyNewSubject);
      else storage.clearDailyNewSubject?.();
      writeApexLocalJson('apex_enem_v3_state_schedule_settings', parsed.scheduleSettings || null);
      writeApexLocalJson('apex_enem_v3_state_schedule_week_plan', parsed.schedulePlan || null);
      writeApexLocalJson('apex_enem_v3_state_schedule_history', parsed.scheduleHistory || null);
...
function renderFocusCard() {
  ...
  if (!focusQueue.length) {
    ...
    const finishedDetail = plan.selectedCount > 0
      ? `Você fechou a fila de hoje: ${plan.completed} feita(s), ${plan.skipped} pulada(s) e ${plan.backlogCount} no backlog para outro momento.`
      ...
      <div class="focus-card focus-finish-card">
        <div class="focus-empty focus-finish">
          ...
          <div class="focus-summary-grid">
            ...
            <span><strong>${plan.backlogCount}</strong><small>backlog</small></span>
          </div>
        </div>
...
// ─────────────────────────────────────────────────────────────
// PLANO DA SEMANA / SIMULADOS / BUSCA / OFFLINE
// ─────────────────────────────────────────────────────────────
...
function buildSimpleWeeklyPlan() {
  const days = getNextSevenDays();
  const reviewCandidates = buildStudyTodayCandidates();
  const newCandidates = buildNewSubjectCandidates();
  const errorNotes = storage.loadErrorNotes();
  const limit = getDailyReviewLimit();
  let reviewIndex = 0;
  let newIndex = 0;
  ...
  const planDays = days.map((date, idx) => {
    const weekday = date.toLocaleDateString('pt-BR', { weekday: 'long' });
    const isWeekend = date.getDay() === 0;
    const tasks = [];

    if (isWeekend && idx !== 0) {
      return { date: getTodayKey(date), label: weekday, rest: true, tasks };
    }

    const reviews = reviewCandidates.slice(reviewIndex, reviewIndex + limit);
    reviewIndex += reviews.length;
    reviews.forEach(item => tasks.push({
      type: 'Revisão',
      title: `${item.topic.name} → ${item.sub.name}`,
      detail: `${item.subject.name} · ${getStudyReason(item)}`,
    }));

    const newItem = newCandidates[newIndex++];
    if (newItem) tasks.push({
      type: 'Assunto novo',
      title: `${newItem.topic.name} → ${newItem.sub.name}`,
      detail: `${newItem.subject.name} · ${normalizePriority(newItem.sub.priority)} · incidência ${newItem.topic.enemWeight || '–'}`,
    });

    if (idx % 2 === 1 && errorNotes[errorIndex]) {
      const note = errorNotes[errorIndex++];
      tasks.push({
        type: 'Caderno de erros',
        title: `${note.subject || 'Geral'} → ${note.topic || 'Erro salvo'}`,
        detail: note.subtopic ? `Reler erro em ${note.subtopic}` : 'Reler erro e correção',
      });
    } else {
      tasks.push({
        type: 'Exercícios',
        title: 'Praticar o assunto principal do dia',
        detail: 'Resolver questões e corrigir com atenção',
      });
    }

    return { date: getTodayKey(date), label: weekday, rest: false, tasks };
  });
  ...
  const plan = {
    ...
    days: planDays,
    backlog: {
      reviews: Math.max(0, reviewCandidates.length - reviewIndex),
      newSubjects: Math.max(0, newCandidates.length - newIndex),
      errors: Math.max(0, errorNotes.length - errorIndex),
    },
  };
...
function buildWeeklyPlanText(plan) {
  const lines = ['ApexENEM — Plano da semana', `Gerado em: ${fmtDateTime(plan.generatedAt)}`, ''];
  plan.days.forEach(day => {
    lines.push(`${day.label} (${day.date})${day.rest ? ' — descanso' : ''}`);
    if (!day.tasks.length) lines.push('- Descanso');
    day.tasks.forEach(task => lines.push(`- ${task.type}: ${task.title} — ${task.detail}`));
    lines.push('');
  });
  lines.push(`Backlog: ${plan.backlog.reviews} revisões, ${plan.backlog.newSubjects} assuntos novos, ${plan.backlog.errors} erros.`);
  return lines.join('\n');
}

function renderWeeklyPlan(plan = storage.loadWeeklyPlan?.() || buildSimpleWeeklyPlan()) {
  const el = $('weeklyPlanContent');
  if (!el) return;
  el.innerHTML = `
    <div class="weekly-plan-summary">
      <div><span>Meta diária</span><strong>${plan.limit}</strong></div>
      <div><span>Backlog revisões</span><strong>${plan.backlog.reviews}</strong></div>
      <div><span>Assuntos novos fora</span><strong>${plan.backlog.newSubjects}</strong></div>
      <div><span>Erros fora</span><strong>${plan.backlog.errors}</strong></div>
    </div>
    <div class="weekly-days-list">
      ${plan.days.map(day => `
        <article class="weekly-day-card ${day.rest ? 'is-rest' : ''}">
          <strong>${esc(day.label)} <small>${esc(day.date)}</small></strong>
          ${day.rest ? '<p>Descanso planejado.</p>' : `<ul>${day.tasks.map(task => `<li><span>${esc(task.type)}</span><b>${esc(task.title)}</b><small>${esc(task.detail)}</small></li>`).join('')}</ul>`}
        </article>
      `).join('')}
    </div>
  `;
...
function openWeeklyPlanModal(forceNew = false) {
  const modal = $('weeklyPlanModal');
  if (!modal) return;
  const plan = forceNew ? buildSimpleWeeklyPlan() : (storage.loadWeeklyPlan?.() || buildSimpleWeeklyPlan());
...
function attachEventHandlers() {
  ...
  $('subjectProgressArea')?.addEventListener('click', e => {
    ...
    const weekBtn = e.target.closest?.('#homeOpenWeeklyPlan');
    if (weekBtn) { openWeeklyPlanModal(); return; }
    const btn = e.target.closest?.('.btn-open-new-subject');
    if (!btn) return;
    const item = flattenAllSubtopics().find(({ sub }) => sub.id === btn.dataset.subId);
    if (item) focusNewSubject(item);
  });
  ...
  $('btnWeeklyPlan')?.addEventListener('click', () => openWeeklyPlanModal());
  $('weeklyPlanClose')?.addEventListener('click', closeWeeklyPlanModal);
  $('weeklyPlanGenerate')?.addEventListener('click', () => {
    renderWeeklyPlan(buildSimpleWeeklyPlan());
    showToast('Plano da semana gerado.');
  });
  $('weeklyPlanDownload')?.addEventListener('click', () => {
    const plan = storage.loadWeeklyPlan?.() || buildSimpleWeeklyPlan();
    downloadTextFile(buildWeeklyPlanText(plan), `ApexENEM_Plano_Semana_${new Date().toISOString().slice(0, 10)}.txt`);
  });
  $('weeklyPlanModal')?.addEventListener('click', e => { if (e.target === $('weeklyPlanModal')) closeWeeklyPlanModal(); });
  ...
  $('currentViewLabel')?.addEventListener('click', () => {
    resetAllFilters();
    renderFilters();
    renderSidebar();
    renderDashboard();
...
function initialize() {
  ...
  if (persisted && Array.isArray(persisted.subjects) && persisted.subjects.length) {
    ...
    if (persisted.filters) appState.filters = { ...appState.filters, ...persisted.filters };
    if (persisted.currentView) appState.currentView = persisted.currentView;
...
// API segura para componentes extras, como o cronograma inteligente.
